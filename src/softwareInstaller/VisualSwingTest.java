...
public class VisualSwingTest {
	...
	private static void initialize() throws IOException {
		...
		final JLabel fetchHint = new JLabel("请输入要核对的文件路径：");
		...
		// 核对按钮
		final JButton fetch = new JButton("核对");
		...
		vText.addFocusListener(new FocusAdapter() {
			...
			public void focusLost(FocusEvent e) {
				...
				if (!Main.getSoftwareVersion().contentEquals(vText.getText())) {
					String inputstr = JOptionPane.showInputDialog(dialog, "要修改版本号，请再次输入管理员密码：：", "请输入密码",
		...
		if (Main.getFuncationSwitch().contentEquals("true")) {
			fSwitch.setText("开");
		...
		fSwitch.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				StringBuffer str = new StringBuffer("5858");
				String inputstr = JOptionPane.showInputDialog(dialog, "要切换功能开关，请再次输入管理员密码：：", "请输入密码",
						JOptionPane.QUESTION_MESSAGE);
				if (inputstr != null && inputstr.contentEquals(str)) {// 修复了点击取消后会报错的bug
		...
		final JButton roleButton = new JButton("登陆");
		...
		roleButton.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if (roleChooser.getSelectedIndex() == 1) {
					...
					String inputstr = JOptionPane.showInputDialog(dialog, "请输入密码", "密码", JOptionPane.QUESTION_MESSAGE);
